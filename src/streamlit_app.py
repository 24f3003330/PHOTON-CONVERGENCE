...
def generate_mock_data(scenario_key, total_hours=168, freq='15Min'):
    ...
    time_index_future = pd.date_range(start=end_time, end=end_time + timedelta(minutes=15), freq='1Min')
    df_future = pd.DataFrame(index=time_index_future)
    df_future['Demand_Forecast'] = df['Consumption'].iloc[-1] * (1 + np.linspace(0, 0.1, len(df_future))) + np.random.rand(len(df_future)) * 0.1
...
def create_energy_flow_chart(df_full, df_future):
    ...
    # Define Time Axis Range (Full 168 hours + 15 min forecast)
    ...
    latest_peak = df_full[df_full.index.hour == action_hour].index.max()
    if latest_peak:
        fig.add_shape(
            type="rect",
            x0=latest_peak.replace(minute=0),
            x1=latest_peak.replace(minute=0) + timedelta(hours=1),
            y0=0, y1=df_full['Consumption'].max() * 1.1,
            line=dict(width=0),
            fillcolor="rgba(255, 165, 0, 0.2)",
            layer="below"
        )
    
    # --- 2. CURRENT DATA TRACES (Plotted using the FULL 168 hours of synthetic data) ---
    ...
    fig.update_layout(
        title='Energy Flow & **ML-Optimized Dispatch** (Scroll to view 168 Hours)',
        xaxis_title="Time",
    ...
    fig.update_xaxes(
        ...
        range=[initial_view_start, end_time] # Initial zoom to last 24H
    )
...
def main_dashboard():
    ...
    st.markdown("The dotted red line shows the ML **Demand Forecast**. The orange highlight shows the next **RL-determined Battery Discharge Window**.")
